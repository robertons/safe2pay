        ...
        if name:
            if name == 'str':
                ...
                self.type = getattr(__import__(
                    f'safe2pay.entities.{name.lower()}', fromlist=[name]), name)
        else:
            self.type = None
        self.format = format
        self.key = key
    ...
    def append(self, item):
        try:
            ...
            if isinstance(item, list):
                ...
                if (self._context):
                    self._context.__metadata__['relasionships'][self._key].append(item.toJSON() if hasattr(item, 'toJSON') else item)
